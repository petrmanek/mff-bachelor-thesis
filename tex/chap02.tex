...
The crossover operator represents an instance of sexual reproduction (also known as \textit{recombination}). Unlike the previous two operators, it requires the input of exactly two chromosomes from the current generation. During the execution of the operator, equivalent parts of selected chromosomes are randomly chosen and exchanged, producing two new chromosomes, both carrying certain traits of their parents.
...
\begin{description}
	...
	A single point is randomly chosen to divide both arrays in two parts. While the first pair of parts is kept at its original position, the second pair is swapped between the chromosomes.

	\item[Two-point crossover]
	In analogous way to the one-point crossover, two points are randomly chosen to divide both arrays in three parts. The middle pair of parts is swapped between the chromosomes, whereas the remaining two pairs are left unmodified.
\end{description}

For tree structures, the crossover is implemented by selecting two random subtrees rooted in nodes, which are descendants of the same base class (either both nodes are action nodes or value nodes specialized on matching types). The crossover is performed when both subtrees are swapped between trees.
